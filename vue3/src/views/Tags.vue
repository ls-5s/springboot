...
const fetchList = async () => {
  try { const res = await getTagsApi(); list.value = res.data } catch { list.value = [] }
}

const handleAdd = () => { editingId.value = null; form.name = ''; showModal.value = true }
const handleEdit = (item: TagItem) => { editingId.value = item.id; form.name = item.name; showModal.value = true }

const handleSave = async () => {
  if (!form.name.trim()) return
  saving.value = true
  try {
    if (editingId.value) { await updateTagApi(editingId.value, form) }
    else { await createTagApi(form) }
...
const handleDelete = async (item: TagItem) => {
  ...
  try { await deleteTagApi(item.id); fetchList() } catch { /* ignore */ }
}
...
<template>
  <div class="page">
    <div class="page-header"><h1>标签管理</h1><button class="btn-primary" @click="handleAdd">+ 新增标签</button></div>
    <div class="card">
      <table v-if="list.length" class="table">
        <thead><tr><th>名称</th><th>创建时间</th><th>操作</th></tr></thead>
        <tbody>
          <tr v-for="item in list" :key="item.id">
            <td><span class="tag-badge">{{ item.name }}</span></td>
    ...
    <Teleport to="body">
      <div v-if="showModal" class="overlay" @click.self="showModal = false">
        <div class="modal">
          <div class="modal-header"><h2>{{ editingId ? '编辑' : '新增' }}标签</h2><button class="close" @click="showModal = false">&times;</button></div>
          <div class="modal-body"><div class="field"><label>名称</label><input v-model="form.name" placeholder="请输入标签名称" /></div></div>
...
.tag-badge { display: inline-block; padding: 2px 12px; border-radius: 10px; background: #f0f0ff; color: #667eea; font-size: 13px; }
